...
- **Fork/template drift guards (pre-commit):** telegram-fork-drift (flags
  ...
  `SKIP=telegram-fork-drift`), template-himmel-plugins (two invariants on
  `docs/setup/settings-template.json`, the single list `adopt.sh`/
  `install-plugins.sh` installs from: every locally-vendored `@himmel`
  marketplace plugin must be enabled in it, and every marketplace named by an
  `enabledPlugins` key must have an `extraKnownMarketplaces` entry —
  `install-plugins.sh` registers ONLY what that map declares, so an
  unregistered marketplace's plugins can never install and the post-install
  presence-verify fails the whole run (HIMMEL-2691). Fail-closed on drift; a
  MISSING input is a legitimate skip, but an input that EXISTS and is
  unreadable fails closed rather than reading as two empty sets), hud-drift
...
- **Doctor check-ID + shell platform-guard gates (pre-commit):**
  ...
  `scripts/himmel-doctor.sh` — every `check_cNN()` must be defined exactly
  once per numeric ID and invoked exactly once in the tail call list; catches
  a silently-shadowed ID from two branches picking the same next-free number,
  which a textually-clean merge does not surface; HIMMEL-2664),
  new-shell-platform-guard (`check-new-shell-platform-guard.sh`, always-run —
  refuses a commit that ADDS a `scripts/**/*.sh` with neither a `.ps1` twin
  nor a documented platform-guard marker in its first 60 lines; the predicate
...
### `check-main-ref-transaction.sh` — main-branch reference-transaction guard (HIMMEL-2095)

This is a **git** `reference-transaction` hook, not a Claude PreToolUse hook —
it is not in `.claude/settings.json` or the `wire-hook-bash.mjs` inventory, and
it could not be: `pre-commit install --hook-type` does not support
`reference-transaction` at all (its own `--help` lists commit-msg,
post-checkout, post-commit, post-merge, post-rewrite, pre-commit,
pre-merge-commit, pre-push, pre-rebase, prepare-commit-msg — this type isn't
among them), which is exactly why it needs its own installer, below.

The rule: refuses an update to `refs/heads/main` whose new oid is not already
an ancestor of `refs/remotes/origin/main`. Creating a local commit on main —
`git commit`, `merge`, `rebase`, or `reset --hard` to an unpublished oid — is
refused; a fast-forward to already-published history (`git pull --ff-only`,
`fetch` + `merge --ff-only`, `reset --hard origin/main`) is not.

Why this hook type: `d05ef8ca` (and three earlier instances) landed directly
on private main via `git commit --no-verify` then `git push --no-verify`,
which skips `pre-commit`, `commit-msg` AND `pre-push` wholesale — so
`check-worktree-isolation.sh`, `check-graph-artifact-branch.sh`,
`check-commit-msg.sh` and `check-push-target.sh` all fire correctly and were
verified working, but never RAN. `reference-transaction` is not one of the
hook types `--no-verify` disables, so it complements those four gates rather
than replacing them. Server-side branch protection is not an option here:
both the branch-protection and rulesets APIs return HTTP 403 "Upgrade to
GitHub Pro" on this private repo.

Installation: `.git/hooks/` is untracked, so the tracked script alone is NOT
the protection. `scripts/hooks/install-main-ref-transaction.sh` (called from
BOTH `scripts/setup.sh` and the lighter `scripts/setup-hooks.sh`, the same two
call sites `install-cr-pre-push-legacy.sh` uses in the pre-push section above)
installs a shim that execs the tracked `check-main-ref-transaction.sh` via
`bash` explicitly (never relying on the target's own exec bit surviving every
checkout/platform). Five review rounds found the installer (and
`/himmel-doctor` check C32, which must stay in sync) re-deriving something by
hand that was already solved elsewhere, wrong in a new way each time:

- **Where does git look for a hook?** `core.hooksPath` set makes git stop
  looking at the common dir at ALL; a RELATIVE `core.hooksPath` resolves PER
  WORKTREE (not shared, breaking the common-dir guarantee); `git rev-parse
  --git-common-dir`'s own relative output is relative to the invoking
  directory, not the worktree top. All three close the same way: ask git
  directly instead of re-deriving its answer — `git rev-parse
  --path-format=absolute --git-path hooks` is git's OWN resolution of "where
  does a hook file belong," already correct for `core.hooksPath` unset,
  relative, or absolute, from any cwd, per worktree. When `core.hooksPath` is
  relative, the installer additionally enumerates every linked worktree
  (`git worktree list --porcelain`) and installs into each one's own
  resolved location, since a single install would otherwise leave every
  worktree but one unprotected — with one stated, accepted limit: only
  worktrees that **exist at install time** are covered; a worktree created
  afterwards has no shim until the installer is re-run (a Suggestion the
  installer documents rather than silently under-covers — see its own header
  — and the honest fix is operational: re-run `install-main-ref-
  transaction.sh` / `setup.sh` / `setup-hooks.sh` after `git worktree add`
  whenever `core.hooksPath` is relative). C32 reports on its OWN checkout
  only; it does not enumerate siblings.
- **How do you embed one string (the target path) in another (the hook
  file) and read it back safely?** Two attempts each broke differently: an
  `eval`-ed `target=...` assignment extracted from the hook file was
  exploitable outright — the marker gating entry into that code path
  authenticates NOTHING (a comment, copyable by anyone), so a hook carrying
  the marker plus a hostile `target=$(...)` executed arbitrary commands
  during a supposedly read-only doctor run. Replacing it with a literal
  `# himmel-main-ref-target: <path>` comment (read verbatim, never parsed)
  closed the code-execution class but broke a third way: a checkout path
  containing a literal embedded newline byte (legal on POSIX filesystems)
  terminates the comment line early and injects a new line of shell into the
  file. The fix removes the question rather than patching it a third time:
  the target now lives in EXACTLY ONE place, `git config --local
  himmel-main-ref.target <path>`, set once by the installer — git's own
  config subsystem already solves "store an arbitrary byte string, retrieve
  it exactly" (verified: a value containing `$`, backticks, `"`, and a
  literal embedded newline all round-trip byte-exact), and it is naturally
  shared across every linked worktree via `--local`. The shim is now 100%
  STATIC (identical bytes at every install site, nothing per-checkout
  interpolated into the file at all) and reads the value back the same way
  at hook-run time; C32 reads it the exact same way too. Nothing is ever
  parsed out of the hook FILE's content any more beyond the plain
  marker-presence check.

C32 verifies the resolved hook is present-with-marker, executable (git
silently ignores a non-executable hook on Unix), and that its configured
target genuinely resolves before reporting OK, and otherwise reports missing
/ foreign (a reference-transaction hook exists but carries no Himmel marker —
refused to overwrite) / non-executable / target-vanished / no-target-
configured. Every one of these is the same lesson: writing the hook file is
not the same fact as git actually running it — a C32 OK proves the marker,
the executable bit, and the resolved-target readability, all at the moment it
ran; it cannot prove none of those has changed since, and it says nothing
about sibling worktrees.

Two deliberate fail-open decisions, so neither gets "fixed" back into a
fail-closed that bricks the repo:

- `refs/remotes/origin/main` unresolved (a fresh clone mid-setup, a
  differently-named remote) → allow, with a warning. There is no published
  main to bypass, so this cannot be this guard's bug class.
- The shim's configured target (`git config --local himmel-main-ref.target`)
  stops resolving (the checkout that ran the installer got moved, renamed, or
  pruned) → allow, with a warning. `reference-transaction` fires on EVERY ref
  update in the repo — `fetch`'s `refs/remotes/*`, stash, tags, every branch —
  so refusing on a broken shim would fail every such operation repo-wide,
  recoverable only by knowing to delete an untracked file under
  `.git/hooks/` that no `git diff` ever shows.

Escape: `MAIN_REF_TRANSACTION_OK=1` as an ordinary PER-COMMAND prefix
(`MAIN_REF_TRANSACTION_OK=1 git commit --no-verify …`) — unlike
`EDIT_ON_MAIN_OK` and every other Claude-side hook bypass, this one does NOT
need the launching-shell rule: `reference-transaction` runs as a direct child
of `git commit`/`git push` itself, so ordinary environment inheritance
already reaches it. Using it appends one line to
`<git-common-dir>/main-ref-overrides.log` (shared by every linked worktree).
`.single-writer` at the repo root opts a repo out entirely (vaults, state
repos that commit straight to main by design) — same convention as
`block-edit-on-main.sh` below.

Known residual, stated plainly rather than left for someone to discover: `git
-c core.hooksPath=/dev/null commit` bypasses this hook (and any client-side
git hook — an inherent limit of the mechanism, not something closable at this
layer). Also residual: `scripts/setup-hooks.ps1`, the Windows twin of
`scripts/setup-hooks.sh`, has no installer call at all, so a Windows station
ships the tracked script with no protection installed (HIMMEL-2638) — the two
bash entry points are covered, the PowerShell one is the one open gap.

Spec: `scripts/hooks/test-check-main-ref-transaction.sh`.
...
### `run-node.sh` / `resolve-node.sh` — dash silent-no-op class (HIMMEL-2692 / HIMMEL-2741 / HIMMEL-2758)

Every chained hook above is launched through `scripts/lib/run-node.sh`, which
Claude Code runs under `/bin/sh` — dash on Debian/Ubuntu — so a bash-only
expansion anywhere in that call chain aborts the file with dash's
"Bad substitution" and takes the FAIL-OPEN path: exit 0, nothing on
stdout/stderr, no guardrail fires. HIMMEL-2692 fixed the bashism in
`run-node.sh` itself; HIMMEL-2741 found the same class one file downstream
(`resolve-node.sh`, which `run-node.sh` sources) — invisible to both the
literal-bashism grep (scoped to the wrong file) and the busybox-based control
(busybox ash implements the bashism dash doesn't). `scripts/lib/test-run-node.sh`
now checks every file the two `run-node.sh` copies source, not just the
launchers themselves, and prefers real `dash` over busybox for the
non-bash-POSIX-shell control.

HIMMEL-2758 is the third member of the class and the first where the
*invocation convention*, not a bashism inside the chain, was the defect. Every
hook command was emitted as
`. "$…/run-node.sh" "$…/run-hook-with-bash.js" <args>`. POSIX leaves `.` with
operands beyond the filename **unspecified**, and dash drops them:

```text
dash -c '. probe.sh a b'   ->  $# = 0
dash -c 'sh  probe.sh a b' ->  $# = 2
bash -c '. probe.sh a b'   ->  $# = 2
```

so on Debian/Ubuntu `run-node.sh` reached its `exec "$_node" "$@"` with an
empty `"$@"` and ran a **bare `node`**. Handed the hook's own JSON payload on
stdin that node parses it as JavaScript and dies with a `SyntaxError` at rc=1;
handed nothing it exits 0. Claude Code treats a PreToolUse rc=1 as a
*non-blocking* hook error, so under either reading the tool call PROCEEDS — the
write fence, the destructive-command block, the secret-read block and the
CR-marker gate were not merely quiet, they were OPEN. Neither reading is ever
rc=2, so it was never a DENY.

The fix has three parts. Both wirers (`scripts/hooks/wire-hook-bash.mjs` for
the 14 `.claude/settings.json` entries, `scripts/hooks/wire-plugin-hook-bash.mjs`
for the 21 `marketplace/plugins/himmel-ops/hooks/hooks.json` entries) emit
`command -p sh "…/run-node.sh" …` and recognise both superseded shapes as
legacy forms, so existing entries MIGRATE on the next run rather than being
refused as foreign. `run-node.sh` (both byte-identical copies) refuses an empty
`"$@"` with one stderr line and **rc=2** — a visible DENY — instead of exec'ing
a payload-less node. And `scripts/lib/test-run-node.sh` gained three legs: the
zero-argument refusal under both bash and dash, an assertion that no live hook
command carries either superseded launcher, and the hostile-PATH leg below.
`.codex/hooks.json` does not reference `run-node.sh`, so the Codex lane needed
no twin change.

`sh` rather than `bash` because HIMMEL-1516 banned the bare `bash` token from
this inventory over Windows PATH roulette (the WSL stub, the 0-byte Store
alias, the "Select an app to open 'bash'" modal). `sh` is not that class:
Claude Code already runs the whole hook command through `sh -c`, so `sh` is
exactly as resolvable as the command line carrying it, and there is no
System32 `sh.exe` decoy.

**`command -p sh`, not a bare `sh`.** The first cut of this fix wired a bare
`sh`, and that traded one silent failure for another. Replacing the `.`
BUILTIN with an external launcher introduces a PATH lookup the dot form never
needed, and a hook shell's PATH is not ours to assume — the pinned-minimal-PATH
GUI/agent launch is the exact environment `resolve-node.sh` exists for.
Measured on a bash-`/bin/sh` station with `PATH=/nonexistent`, outer shell
started by absolute path:

```text
bash -c '. "$RUN_NODE" --version'             ->  rc=0    v24.20.0
bash -c 'sh "$RUN_NODE" --version'            ->  rc=127  sh: command not found
bash -c 'command -p sh "$RUN_NODE" --version' ->  rc=0    v24.20.0
```

The dot form survived because `resolve_node()` step 3 probes **absolute**
well-known locations (`/usr/bin`, homebrew, …) and never consults PATH for node
at all — so a bare-`sh` wiring would have swapped a dash-only silent no-op for
a restricted-PATH **dead chain**, the same fail-the-wrong-way class this ticket
exists to close. The `else node …` branch cannot catch it: that branch is
reached only when `run-node.sh` is MISSING, and a bare `node` is equally
unresolvable there. `command -p` is the POSIX "search the standard utilities
PATH" form, so the launcher resolves from the system default whatever PATH was
inherited. `test-run-node.sh` pins both PATH shapes (unset — where `exec`'s
confstr fallback to `/bin:/usr/bin` means even a bare `sh` would have worked,
which is why that leg alone did not catch this — and set-but-useless), with a
negative control asserting the superseded bare `sh` still dies rc=127 there.

The same commit dropped `run-node.sh`'s `$(dirname "$0")` fork for the builtin
`${0%/*}`: under that same hostile PATH the fork failed with `dirname: command
not found` on stderr and silently demoted the direct-execution lane. A hook
launcher must not depend on any external utility resolving.

Busybox ash is **not** a control for this class either — it implements
dot-with-operands, exactly as it implements the `${var//pat/repl}` expansion of
HIMMEL-2741. Only real `dash` reproduces both. (It IS a useful control for the
launcher's portability, which is a different question: `test-run-node.sh` runs
the wired shape under dash, busybox ash and `zsh --emulate sh`, and `command -p`
resolves under all three.)
...
`gh pr create` on this hook's own bug. One deliberate exception (HIMMEL-2720):
once a marker is present for the resolved branch, an unreadable HEAD fails
CLOSED — a hook that cannot read HEAD must not wave a pending-CR PR through.
...
- the shared `pre-push` **chains** to the repo hook (tokensave's
  `chain-repo-hook` shim was exactly this, before tokensave was dropped —
  HIMMEL-2581) → the gate fires; informational note.
...
### `block-write-into-main-checkout.sh` — Bash-mediated primary-checkout write fence (HIMMEL-2526)

PreToolUse on `Bash`. The PREVENTION half of the HIMMEL-2526 pair —
`detect-dirty-primary.sh` (documented in the PostToolUse section below) is
the DETECTION half. `block-edit-on-main.sh` above only fires on
`Edit|Write|MultiEdit|NotebookEdit`, so a Bash-mediated write (`cat >`,
`sed -i`, a python heredoc) reaches the primary checkout unfenced; this hook
closes that gap by denying a Bash command whose TARGET path resolves into a
protected checkout (a repo's primary checkout, per the same git-dir ==
git-common-dir discriminator `block-edit-on-main.sh` and
`block-edit-live-settings.sh` use) rather than the worktree the session is
scoped to.
...
this doc and the ticket record that is itself worth noting, since the two
shapes listed here were the ones a reader would have believed complete. All
six are closed by this PR. The record stays rather than being deleted: a
fence believed to cover a shape it does not is worse than no fence, and a
record of what was actually checked is the mitigation.

| # | Shape | Ticket | Proving row |
|---|---|---|---|
| 1 | Quoted/commented heredoc opener — `echo '<<EOF'`, `echo "<<EOF"`, `# <<EOF` — used to start heredoc body-blanking and ERASE a following real write | HIMMEL-2591 | adjudicate5 rows 1a, 1b, 1c |
| 2 | `mv <primary>/tracked.txt $DEST` — a dynamic destination used to suppress the SOURCE check | HIMMEL-2592 | adjudicate5 row 2a |
| 3 | `tee /dev/null >/primary/f` — an attached redirect operator consumed as a second tee file operand | HIMMEL-2595 | adjudicate6 rows 1a, 1c |
| 4 | `echo x >\| <primary>/f` — the `>\|` clobber operator split at the pipe by the clause splitter | HIMMEL-2592 | adjudicate7 rows 1a, 1b |
| 5 | `rm`/`mv`-source of a symlink INSIDE the primary pointing out | HIMMEL-2592 | adjudicate7 rows 2a, 2b; adjudicate8 rows 5a/5b/5c MIRROR |
| 6 | `tee /dev/null > /dev/null` from the primary — a false POSITIVE | HIMMEL-2595 | adjudicate7 rows 3a, 3b |

Also closed here: HIMMEL-2597 — the `guardrails/lib.sh` symlink resolver's
hop cap returned the still-unresolved path WITH SUCCESS on exhaustion, and
each hop's own containing directory was never canonicalised — proven by
adjudicate8 rows 1a and 2a plus the `test-lib.sh` rows. Each row reproduces
against a real linked worktree with passing negative controls:

    PROBE_WT=<worktree> bash "$HOME/.himmel-2592-adjudicateN.sh" <scratch-dir-under-$HOME>

Instance 2 was filed as a DESIGN ticket, not a bug: the third occurrence of
one pattern (an operand check nested inside a sibling's resolution guard,
already fixed twice in the positional and `-t` destination branches), and the
fix states the invariant none of those earlier fixes did — below.

**The design invariants that close the class** (what stops a seventh
instance, not the six patches themselves):
- An operand/opener/splitter loop that meets a token it cannot classify FAILS
  CLOSED or hands it to the redirect arm — it never silently consumes it.
- Every operand the extractor can resolve STATICALLY is checked
  INDEPENDENTLY of whether any sibling operand resolved.
- Path resolution is chosen by the OPERATION, not by the path: a verb acting
  on the directory ENTRY (`rm`, an `mv` source) does not dereference the
  final symlink; a verb writing THROUGH it does.
- ENTRY semantics apply ONLY to a BARE final-component symlink. Anything
  after the link in the raw token — a trailing `/`, `/.`, `/..`, `/name`, or
  a glob — traverses it, so the operation is FOLLOW.
- A dynamic (`$VAR`) operand fails open on ITSELF ONLY — never consumed as
  another class, never suppressing a sibling. This is a deliberate divergence
  from HIMMEL-2592's literal "fails closed" wording, ratified because the
  HIMMEL-2526 §2 spec says dynamic targets fail open and denying every
  variable operand would make the fence unusable.

**Deliberately removed denies.** Three were removed as ONE ratified class:
`rm <wt>/filelink`, `mv <wt>/filelink <wt>/x`, `rm -r <wt>/dirlink` — an ENTRY
verb acting on a bare final-component symlink whose ENTRY lives in an
unprotected checkout cannot reach the referent, so the old deny protected
nothing. Verified at filesystem level with counter-controls (`echo x >
<filelink>` clobbers the referent's contents; `rm -r <dirlink>/` removes
them) — the counter-controls are what make the "referent untouched" rows
evidence rather than a blind fixture. Each removal is pinned in the suite
beside its MIRROR row: the same verb on a link whose entry lives INSIDE the
primary still denies.

**Accepted cost.** HIMMEL-2597's fail-closed behaviour means
`guard_canon_path` returns rc=1 on a symlink chain longer than **40** hops —
parity with the kernel's ELOOP limit, which is effectively what `realpath -m`
honours in the Write-tool fence (`block-edit-on-main.sh`) — so a chain that
long, even one that never leaves the worktree, is refused with "the target
path could not be canonicalised". A chain up to 40 hops resolves normally and
is classified on its real target: a 10-hop worktree-local chain ALLOWS
(adjudicate8 row 1c), a 41-hop one still DENIES (row 1d), as does a 41-hop
chain into the primary (row 1e). A chain that long is pathological, and the
ticket ratified fail-closed over silently emitting an unresolved path — but
the cost is a false positive on a genuinely deep worktree-local chain, named
here rather than left for a reader to discover by surprise.

Related open scope limits on the DETECTION half: HIMMEL-2571 (`status -uno`
does not see newly CREATED files) and HIMMEL-2590 (the baseline cursor
advances only on a reporting run). One level down, the SAME silent give-up
shape recurs and is NOT closed here: HIMMEL-2600 — `block-edit-on-main.sh`'s
own `canon()` (GNU `realpath -m`) returns an unresolved path at rc=0 on a
symlink CYCLE, deferred as out of this PR's diff. It does not share the
depth-cap bypass above — `realpath -m` resolves a 12-hop chain correctly
(its own limit is ELOOP, around 40) — so the exposure is a cycle only, not a
merely-deep chain.

Still-true limitations of this fence, unchanged by this PR (see the hook's
own header): no `cd` modelling, interpreter bodies are not parsed for writes,
and the verb scan is prefix-blind (`sudo cp` is missed) — except `tee`, which
carries a CLOSED prefix-skip list and so does handle `sudo tee`/`env
tee`/`FOO=1 tee`. `-T`/`--no-target-directory` IS handled (this bullet used
to say the opposite): for `mv` it forces the destination to plain ENTRY
resolution instead of directory-follow (bundled forms like `-fT` included),
and for `ln` the same flag — alongside `-n`/`--no-dereference` — does the
same. For `cp` specifically it is deliberately NOT applied: real `cp -T`
against a directory-shaped destination refuses (rc=1) and writes nothing, so
treating it as a mode-flip would be a pure false positive.
...
**Operator ruling 2026-09-07: the GitHub App is the CodeRabbit surface. There is
no CLI.** `scripts/cr/coderabbit-review.sh` (the CLI wrapper) and
`scripts/cr/coderabbit-gate.sh` (the `/pr-check` conserve-or-run gate) are
DELETED, together with `CODERABBIT_BIN`, `CODERABBIT_WSL`,
`CODERABBIT_TIMEOUT_SECS`, `CODERABBIT_CLI_DISABLE` and `CODERABBIT_ALLOW_WSL`.
HIMMEL-2683 ("arm the CodeRabbit CLI") is closed won't-do.

**Why.** The CLI was a second charge against the SAME rate-limited CodeRabbit
account for a signal the App already posts on the PR — and worse, each
CodeRabbit-driven fix round it forced also cost an extra **paid codex panel
row**, because the cross-model marker gate needs a non-Claude availability row
at the NEW head. Findings are commented on and tracked where they belong: on
the PR, in GitHub.

**What replaces it: nothing, pre-PR.** The critic panel and the codex
adversarial pass carry `/pr-check`; CodeRabbit enters after `gh pr create`.
That is a real change in coverage for a repo with no App — such a repo now gets
...
1. **Zero unresolved review THREADS. A green check-run is NOT clean.** The
   ...
   merges check-runs and statuses and says nothing about whether anyone resolved
...
responding cross-model reviewers before it would clear a diff that changes the
CR/merge gate infrastructure itself. The CodeRabbit CLI was that second seat, and
`scripts/cr/critics.json`'s panel is codex-only (the `glm` row was retired
2026-08-18, HIMMEL-1904). With no second seat the quorum cannot be met, so that
lane now REFUSES gate-infra diffs outright — the same disposition it already
produced whenever the CLI was absent, which was the common case. Gate-infra
changes go through the interactive `/pr-check` (Claude backstop). Adding a second
non-Claude row to `critics.json` re-opens the lane.

### Codex adversarial pass — re-armed on high-risk diffs only, once per branch (HIMMEL-2707)

**The one-suite-run invariant.** The after-report's `== Summary ==` PR comment
is the ONE suite-run artifact any PR gate consumes — nothing in the merge or
CR path re-runs `scripts/ci/run-shell-tests.sh` against a head that already
carries one. `scripts/console/base-status.sh:257-316` is the implementation:
it reads the after-report Summary keyed by head SHA + fence scope and refuses
to certify a fence PASS unless a Summary exists at that exact head, is not
`TRUNCATED` (a budget-expired run), was not filtered by `--changed-since`, carries
a parseable `FAIL: N` count, and its `scope:` line actually covers the fence
being certified — any of those failing reads PENDING, never a silent pass.
This is a description of an invariant the code already enforces, not a new
gate: no reviewer, critic, or CR lane in this repo — including the pass below
...
### `record-primary-baseline.sh` — primary-checkout dirty-baseline recorder (HIMMEL-2526)

Plugin-delivered via the himmel-ops `hooks.json`, placed right after
`record-hook-integrity.sh`. Records the PRIMARY checkout's tracked
`git --no-optional-locks status --porcelain -uno` output at session start,
keyed by session_id, to
`${HIMMEL_PRIMARY_BASELINE_DIR:-$HOME/.claude/himmel/primary-baseline}/<session_id>.primary-baseline`.
This is the DETECTION half's baseline: `detect-dirty-primary.sh` (the
PostToolUse companion below) diffs each Bash call's current status against
it, so a file already dirty when the session started is never reported.

Modelled closely on `record-hook-integrity.sh`'s conventions (best-effort,
every early exit is `exit 0` writing nothing, same `CLAUDE_PROJECT_DIR`/
`jq`/`git` checks, same session_id shape guard) but writes to a SEPARATE
directory and is intentionally NOT chmod 0400 — unlike the integrity pin,
this baseline is a live cursor that `detect-dirty-primary.sh` rewrites
during the session every time it reports a new dirty path, so the same path
is reported once rather than on every subsequent Bash call. It never writes
into `HIMMEL_HOOK_INTEGRITY_DIR` — that directory belongs exclusively to
`record-hook-integrity.sh`'s own ticket (HIMMEL-2528).

Resolves the PRIMARY via `scripts/guardrails/lib.sh`'s `primary_checkout_root`
(compares `git rev-parse --absolute-git-dir` against `--git-common-dir`),
so a SessionStart fired from inside a linked worktree still records the
primary's own state, not the worktree's. Write via temp file + `mv` so a
reader never observes a partial file.

Spec: `scripts/hooks/test-record-primary-baseline.sh`.

### `detect-dirty-primary.sh` — primary-checkout dirty-write detector (HIMMEL-2526)

PostToolUse on the `Bash` matcher. The DETECTION half of the HIMMEL-2526 pair
— `block-write-into-main-checkout.sh` (documented in the PreToolUse section
above) is the PREVENTION half. Exists because `block-edit-on-main.sh` is only
wired on `Edit|Write|MultiEdit|NotebookEdit`: a Bash-mediated write (`cat >`,
`sed -i`, a python heredoc) bypasses it entirely. The motivating incident: a
dispatched subagent wrote four files into the primary checkout via Bash while
it was on `main`, and nothing noticed for 18 minutes.

**DELIBERATELY UNGATED by any worker marker.** An earlier draft gated this on
`HIMMEL_WORKER=1`; that was withdrawn and must not come back — `HIMMEL_WORKER`
is exported only by the telegram lanes, `scripts/lib/claude-headless.sh` never
sets it, and an Agent-tool subagent inherits the parent's environment
unmarked. The incident worker carried no marker, so a marker-gated detector
would never have fired on the very shape it exists to catch.

On every Bash call: if no baseline file exists for this session, exits 0
silently (fail-open — the recorder is advisory and may legitimately have
written nothing). Otherwise resolves the PRIMARY the same way the recorder
does and re-runs `git --no-optional-locks -C <primary> status --porcelain
-uno` (~single-digit ms on a normal checkout — affordable on every Bash
call). **A failed invocation is never interpreted as clean** — it logs once
per session (a marker file beside the baseline, not stderr noise on every
call) and exits 0.

Diffs the current porcelain output against the baseline by exact LINE, not
by path: only lines present now and absent from the baseline are reported,
so a status-code change on an already-dirty file (` M` → `MM`) still counts
as new. Nothing new → exit 0 silently. Something new → **rewrites the
baseline to the current output**, THEN reports via `exit 2` with the message
on stderr — this repo's PostToolUse advisory convention (the tool already
ran; see `check-hook-file-parse.sh`). The rewrite is what makes a newly
dirty path reported ONCE rather than on every subsequent Bash call for the
rest of the session, while a later, different change is still caught
against the rewritten baseline. The message names the PRIMARY path, the
specific new dirty path(s), and what to do (the write belonged in a
worktree; check whether a dispatched worker was handed an absolute primary
path; HIMMEL-2526).

Spec: `scripts/hooks/test-detect-dirty-primary.sh`.
...
Ledger-driven removal (HIMMEL-3332 S6): after the fence, `uninstall.sh` loads the provenance ledger (`${HIMMEL_PROVENANCE_DIR:-$HOME/.himmel}/provenance.jsonl`) through `scripts/lib/provenance-read.sh` and folds it into one verdict per unit: remove what himmel created, restore what it replaced from the recorded backup, keep what pre-existed, is already gone, was modified since install (prompt on a TTY) or has no backup. The read fails closed: a missing, unparsable or unfoldable ledger (or no `jq`) keeps the six overwrite-prone rows — plugins, marketplaces, `statusLine`, `env.HANDOVER_DIR`, the claude-hud config, adopter script copies — and prints a hand command for each. `--dry-run` writes nothing, the ledger included; outcome rows are appended only when the ledger loaded. `--purge-state` deletes the ledger and its backups last, and only after a clean (not halted) run; `--keep-backups` skips the backup prune. Coverage: `scripts/test-uninstall-provenance.sh`, `scripts/lib/test-provenance-read.sh`, and the no-ledger regression in `scripts/test-uninstall.sh`.
...
Subagent transcripts are NOT flat files under the project directory. A
session's own transcript is `~/.claude/projects/<slugified-cwd>/<sessionId>.jsonl`,
but every subagent it dispatches writes to
`~/.claude/projects/<slugified-cwd>/<parentSessionUUID>/subagents/agent-<agentId>.jsonl`.
Any usage attribution that globs only `*/*.jsonl` therefore sees zero sidechain
turns and is structurally blind to subagent spend — which the HIMMEL-2653
measurement found to be 50.06% of a 4.40e9-token 24-hour window
(`general-purpose` alone 49.85%). Recurse (`**/*.jsonl`), and deduplicate on
`requestId`: one streaming request emits ~1.83 JSONL lines carrying identical
usage counters, so counting lines instead of requests inflates the total by
~83%.

`scripts/lib/bank-preflight.sh`'s fleet-size cap only REFUSES a caller that
declares `CADENCE_BANK_LAUNCH=1` (an actual arm/launch); a plain bank read
(a `/pr-check` critic pass, a statusline probe, a hook-smoke test) gets the
same verdict and FLEET line but is never refused by it — detail:
[lane-calibration.md § Launch vs. read (HIMMEL-2789)](lane-calibration.md#fleet-size-cap--a-provisional-4-grounded-in-the-incident-himmel-2765).
...
`scripts/hooks/*.sh` / `scripts/guardrails/*.sh` script to its git-committed
blob hash at HEAD, once, at `SessionStart` — session-start-absolute. That
meant a wired hook change landing on main between a worker session's start
and a later call **hard-blocked every already-running session**, even a
fully legitimate merge: the on-disk bytes no longer matched the pin, forever,
for the life of that session.

The pin is no longer session-start-absolute. On a mismatch, the launcher
(`run-hook-with-bash.js`'s `verifyProjectHookIntegrity()`) now **re-pins**
instead of denying forever, iff all three hold:
1. the on-disk bytes equal the blob at the **tip of the record's stored
   `anchor_ref`** (`refs/remotes/origin/<default branch>`, resolved once via
   `scripts/guardrails/lib.sh`'s `default_branch()` and recorded by
   `record-hook-integrity.sh` alongside the pins, schema v2);
2. the record's previously-stored `anchor` commit is an **ancestor** of that
   tip (`git merge-base --is-ancestor` — monotonic: a rewind of
   `origin/<default>` denies, it never re-pins backward);
3. the **previously-pinned blob** for that path is reachable in that ref's
   history (`git log --full-history --find-object=…`) — the new content did
   not just appear at the tip unrelated to what this session already trusted.

On accept, the launcher advances both the pin (to the new on-disk blob) and
the stored `anchor` (to the new tip) in one lock-serialized transaction
(`scripts/hooks/hook-integrity-lock.sh`), so two racing sessions cannot each
half-write the record — a dead lock owner's directory is reclaimed by
**renaming** it to `<lock>.dead.<pid>.<random>` before removal, never by an
in-place `rm -rf`, so exactly one contender wins the steal; a stray
`*.lock.dead.*` directory next to the lock is that graveyard path, safe to
ignore or clean up. An **unmerged, feature-ahead** hook change — the
on-disk bytes differ from HEAD, but that blob is not the one that would be
found by walking `anchor_ref`'s history — still denies. That is the intended
degrade: only a change that genuinely reached the trusted branch re-pins;
worktree drift or a not-yet-merged hook edit does not.

**New worker-lane fence — the anchor-tamper class in
`block-glm-external-writes.sh`.** Re-pinning off `origin/<default>` makes
`refs/remotes/origin/<default>` **security-relevant** for the first time: the
re-pin step now trusts whatever that ref currently resolves to as the
definition of "legitimately committed hook content." `block-glm-external-
writes.sh` gained a second worker-lane class (immediately after the
HIMMEL-2085 pin-dir write-fence class, same gate, same "worker-ness not lane"
scope) denying the command shapes a dispatched worker could use to make that
ref resolve to attacker-chosen history:

| Trigger | Scope | Verdict | Bypass |
|---|---|---|---|
| A `Bash`/`PowerShell` command matching either (i) a keyword/substring hit — `refs/remotes/`, `remotes/origin/`, `symbolic-ref`, `remote set-head`, `remote set-url`, `remote add`, `remote remove`, `remote rm`, `remote rename`, `update-ref`, `insteadOf`, `remote.origin.url` — anywhere in the (lowercased) command text, or (ii) a command-position `git [global-flags] (fetch\|push\|pull)` shape carrying an explicit refspec whose **destination** writes remote-tracking space — `refs/remotes/…`, or an `<a>/<b>` shorthand whose `<a>` is a real remote/tracking namespace in this repo — regardless of which remote is named, and including via `git -C <dir>` | Any `worker_lane` session — GLM (`ANTHROPIC_BASE_URL` = api.z.ai) **or** a native dispatched worker (`HIMMEL_WORKER=1`); a headed operator session never reaches `worker_lane` and is unaffected | Deny, rc=2 | `HIMMEL_HOOK_INTEGRITY_BYPASS_OK=1` set in the **launching** shell (same bypass var as the HIMMEL-2085 pin-dir class — one escape hatch covers both hook-integrity fences; since HIMMEL-3396 it is honoured only inside a linked worktree of the session's recorded repo and audited, so the primary checkout keeps both fences on) |

**Class (ii) gates on the destination, not on the remote.** It originally
matched an enumeration of local-path remote spellings, and that axis was
defeated four times: quoted remotes (codex-6), a bare `..`, bare relative
directory names (codex-3), and finally the *configured* `origin` itself —
`git fetch evil.git x:origin/main`, `git fetch sub/dir x:origin/main` and
`git fetch origin x:origin/main` all measured rc=0 ALLOW before this rewrite,
and all three forge the tracking ref exactly as well as `git fetch . …` does.
The fence now classifies the refspec's destination — `refs/remotes/…` denies
unconditionally, `refs/…` (heads, tags, …) allows, a bare `<a>` allows as a
local branch name, and an unqualified `<a>/<b>` denies **iff `<a>` names
remote-tracking space in this repo** — so the remote argument stopped being
load-bearing and no unlisted spelling can slip past. The scan is per command
**segment** (split on `;`, `&`, `|`, parens), and within a segment the refspec
may sit anywhere, which is what covers `--refmap=<spec>`, `--refmap <spec>`
and `-c remote.origin.fetch=<spec>` without a per-flag rule.

The `<a>/<b>` arm has to consult the repo because `origin/main` and a
slash-bearing *branch* name are the same shape, and himmel mandates
`type/slug` branches — a blanket deny false-refused `git push origin
HEAD:feat/x`, the most common push in the repo. `<a>` is resolved against the
union of `git remote` and the first segment of every existing
`refs/remotes/<a>/…` ref in `$CLAUDE_PROJECT_DIR` (the second source matters
because `git remote remove origin` is *not* fenced by class (i), and a stale
`refs/remotes/origin/main` survives it and still DWIMs). This is not a return
to the enumeration the class abandoned: that guessed at an open-ended space of
ways to *write* a remote, where a fifth spelling always exists; this reads a
small closed set out of git itself, and a worker cannot extend it because
`git remote add`, `set-url`, `remove`/`rm` and `rename` are all denied by
class (i). Those last three were a **spec gap**: this ticket's threat model
named `remote set-url|add|rename|remove`, but the prescribed class (i) fence
list omitted `rename` and `remove`, and the implementation followed the
narrower list — so both measured rc=0 ALLOW until this round (`rm` is git's
own accepted alias for `remove`, per git-remote(1), and is fenced with it).
Removing or renaming `origin` is not a footnote here: it collapses the very
set this arm consults, while a stale `refs/remotes/origin/main` survives and
still DWIMs — which is why the tracking-namespace half of the union stays in
place as defence in depth even now that the verbs are fenced. Read-only
`git remote` and `git remote -v` match none of the verbs and stay allowed.

Git is spawned **only** from this arm — after a command-position fetch/push/pull with
an explicit two-segment destination — and at most once per hook invocation, so
an ordinary Bash call never pays for it. If the set cannot be established at
all (not a repo, git missing, non-zero exit) the arm **fails CLOSED**: the
fence refuses what it cannot prove safe, and failing open would hand a bypass
to any worker that could make the lookup fail.

**Deliberately still allowed:** `git fetch origin`, `git fetch origin main`,
`git fetch --all`, `git push origin feat/x`, `git pull`. These update tracking
refs too, via the remote's *configured* default refspec — but they carry no
explicit tracking destination in the command text, and that absence is the
discriminator. `git fetch origin main:main` (bare local branch) and
...
reproduced: pre-fix, `git fetch "." x:origin/main` measured rc=0 ALLOW — a
command-TEXT fence sees the raw text before the shell's own quote removal, and
the bare-spelling-only pattern never matched a quoted `.`). Normalization
resolves a single-character backslash escape (`\.` → `.`) and then drops any
remaining `"`/`'` characters outright, which also collapses adjacent-quote
concatenation (`"".""`) onto the same bare spelling for free — every token
this class inspects is a bare path/ref segment (`.`, `/`, alnum, `-`, `_`)
that contains no quote character itself, so this can only widen a later
match, never narrow it. Declared residual: bash ANSI-C quoting (`$'...'`) is
not decoded — resolving its escape sequences risked a wrong decode turning
into a false ALLOW — so a `$'` appearing **anywhere** in a fetch/push/pull
segment (remote side or destination side; `$'origin\057main'` would otherwise
survive normalization with no literal `/` for the destination classifier to
see) is denied outright instead, regardless of what it would decode to.

Class (i) is a plain, **unanchored** substring scan, not command-position
matching — same non-anchored philosophy as the HIMMEL-2085 pin-dir class's
own `pin_text_hit`, and a deliberately broader net than the rest of this
file's command-position arms. Accepted consequence, test-pinned: a commit
message that merely *mentions* one of these phrases in prose now also
denies (`scripts/hooks/test-block-glm-external-writes.sh`'s "glm prose remote
set-url (HIMMEL-2528 accepted overmatch)" case) — the same fail-closed
direction as every other "Accepted OVER-blocks" row this file's header
already documents for the pre-existing GLM-lane classifiers.

**Declared residual — OPEN.** This is cheap coverage, not closure, and it is
the class that makes `origin/<default>` a trusted input, so the residual is
stated explicitly rather than left implicit:
- A `.git/config` edit performed through a path this fence never inspects
  (a text-editing tool call, or a script whose *content* this class does not
  scan), followed by a plain `git fetch origin`, is invisible — the fetch
  command itself then names nothing suspicious. (`git config remote.*.url`
  IS caught by class (i) when it appears as literal command text — this gap
  is specifically an edit that never appears as command text at all.)
- `packed-refs` / `.git/refs/remotes/` written directly through a path never
  spelled out in the command text this class inspects (redirection into a
  file already inside a worker's Edit grant, or any tool-call surface this
  hook does not mediate) is invisible the same way the HIMMEL-2085 pin-dir
  class's own residual is.
- An **implicit-refspec** fetch (`git fetch origin`) from a remote whose URL
  is already hostile is out of class (ii)'s reach by construction — the
  destination gate has nothing to match. What keeps that gap narrow is class
  (i): `git remote add`, `git remote set-url`, `git config remote.origin.url`
  and `-c url.*.insteadOf` are all denied there (verified: `git remote add
  evil /tmp/decoy` and `git remote set-url origin /tmp/decoy` both measure
  rc=2), so a worker cannot register or repoint a remote as command text
  first. It does not close the bullet above — a `.git/config` edit that never
  appears as command text still reaches the same end.
- Anything an OS-user-level attacker does entirely outside the tool-call
  surface stays out of scope, per this file's header.

Closure is a **signed hook-generation manifest** — hook content trusted by
signature over the committed tree, not by whatever `origin/<default>`
currently resolves to — tracked as **HIMMEL-2572**.

#### Round 3 — class (i) is quote-normalized too (codex-2)

Everything above about quoting normalization described **class (ii) only**.
Round 1 added `anchor_unquote()` to the destination/remote match and left
class (i)'s keyword scan on raw command text — one door closed, its twin left
open. Measured on a native worker lane (`HIMMEL_WORKER=1`), the bypass covered
the **whole class-(i) keyword set**, not just `set-url`:

| command | pre-fix | now |
|---|---|---|
| `git remote set-url origin /tmp/decoy` | rc=2 | rc=2 |
| `git "remote" "set-url" origin /tmp/decoy` | **rc=0** | rc=2 |
| `git remote 'set-url' origin /tmp/decoy` | **rc=0** | rc=2 |
| `git remote se\t-url origin /tmp/decoy` | **rc=0** | rc=2 |
| `git "remote" "add" evil /tmp/d` | **rc=0** | rc=2 |
| `git remote "remove"` / `'rm'` / `re\name` | **rc=0** | rc=2 |
| `git sym"bolic-ref" …` / `git up"date-ref" …` | **rc=0** | rc=2 |
| `git config --global url.…​.instead"Of" …` | **rc=0** | rc=2 |
| `git config remote."origin".url /tmp/decoy` | **rc=0** | rc=2 |
| `echo x > .git/"refs/remotes"/origin/main` | **rc=0** | rc=2 |

Class (i) now greps the **same normalized copy** class (ii) already used —
one `anchor_unquote()` shared by both, deliberately not a second normalizer
that could drift back apart exactly the way these two did.

**This corrects the safety argument stated for the round-2 destination gate
above.** Two paragraphs earlier in this section lean on "`git remote add` and
`git remote set-url` are both denied by class (i)": the `<a>/<b>` lookup
reading the live remote set, and — the one that matters — the
implicit-refspec residual bullet. **Until this round that claim held only for
the unquoted spellings.** The full pre-fix attack was two commands the hook
allowed outright:

```
git remote 'set-url' origin /tmp/evil.git   # rc=0 pre-fix
git fetch origin                            # rc=0 BY DESIGN, and must stay so
```

The second is the implicit-refspec form class (ii) must never deny, so the
first was the *entire* defence. Class (i) is therefore not a redundant
"cheap coverage" layer on this path — class (ii) is uncloseable against it
alone. The dependency is pinned mechanically, not just in prose: the
`class (i) precondition for the destination gate` rows in
`test-block-glm-external-writes.sh` assert both chains deny, and go red under
a class-(i)-only revert **while every class-(ii) case stays green**.

**ANSI-C (`$'...'`) treatment for class (i).** The residual above is unchanged
— escape bodies are still not decoded, because a wrong decode risks a false
ALLOW. The fail-closed deny that answers it **widened** from a
fetch/push/pull segment to **any command-position `git` segment**, since
`git remote $'\x61dd' evil /tmp/d` spells a class-(i) keyword just as well as
`$'.'` spells a remote (rc=0 ALLOW pre-fix). It is deliberately *not* scoped
to an enumeration of the fenced subcommands: that is the same axis class (ii)
abandoned after losing on it four times, and it cannot work here anyway — the
subcommand token is itself quotable (`git $'\x72emote' add evil`, also rc=0
pre-fix). Accepted cost: a worker-lane `git commit -m $'a\nb'` is refused.
Residual, stated not chased: like class (ii)'s shapes this arm is
command-position anchored, so `rtk git remote $'…'` is not seen; class (i)'s
unanchored keyword scan still covers wrapper invocations for every form
`anchor_unquote()` does normalize.

**Accepted overmatch, widened.** Two spellings changed direction this round
and are pinned as such rather than argued away: `git commit -m "the remote
'add' flow is fenced"` and `git commit -m $'a\nb'` now deny. This is not a new
*category* — the unquoted twin (`git commit -m "docs: note the remote set-url
tripwire"`) already denied before this round and has been test-pinned since.
No substring class can separate `git remote 'add' evil` from a commit message
containing the same bytes without being scoped to a git-command shape, and
scoping it that way would drop the `.git/refs` rows in the table above, which
are not git commands at all. `HIMMEL_HOOK_INTEGRITY_BYPASS_OK=1` in the
launching shell remains the escape hatch.

#### Round 4 — two one-command anchor forgeries, and the fifth axis (codex-2, codex-3)

Both were reproduced on a native worker lane (`HIMMEL_WORKER=1`) against the
round-3 hook, and both were **live one-command bypasses**: the denied step is
the only defence, because the follow-up that completes the forgery is the
by-design-allowed `git fetch origin`.

| command | pre-fix | now |
|---|---|---|
| `git remote -v set-url origin /tmp/evil.git` | **rc=0** | rc=2 |
| `git remote --verbose add evil /tmp/d` | **rc=0** | rc=2 |
| `git remote --verbose 'set-url' origin /tmp/evil.git` | **rc=0** | rc=2 |
| `"git" remote $'\x61'dd origin /tmp/evil.git` | **rc=0** | rc=2 |
| `git remote $'\x61'dd origin /tmp/evil.git` | rc=2 | rc=2 |
| `git remote set-url origin /tmp/evil.git` | rc=2 | rc=2 |

**codex-2 — options between `remote` and its verb.** `git remote` accepts
options *before* its subcommand, so class (i)'s six
`remote[[:space:]]+<verb>` patterns, which require adjacency, saw no verb at
all. Every fenced verb was reachable this way, not just `set-url`. The fix
**tokenises**: after a `remote` token, option tokens (anything shaped `-*`)
are skipped and the **first non-option token** is taken as the verb. No list
of options exists anywhere in the code — whatever option git adds next is
`-`-prefixed and is skipped without an edit. Token equality also retired the
`([^a-z0-9_-]|$)` boundary hack that the two-character `rm` verb needed.

The over-block risk is the whole point of the "first non-option token"
wording: `git remote -v` with **no verb after it** is a listing, not a
metadata write. With no non-option token left, nothing is ever compared
against the verb list, so bare `git remote`, `git remote -v` and
`git remote --verbose` all stay allowed, and read-only verbs (`show`,
`get-url`) fall through as non-option tokens that are simply not in the list.
Those, plus `git status`, `git fetch origin` and `git push origin feat/x`,
are re-measured at rc=0 against the fixed hook and pinned individually.

**codex-3 — the ANSI-C arm's literal `git` anchor was quote-evadable.** That
arm must read **unnormalized** text, because `anchor_unquote()` strips the `'`
that the `$'` marker is made of. Round 3 served that requirement and the
command-position anchor from the same raw read, so the anchor was a literal
unquoted `git`: `"git" remote $'\x61'dd …` walked past the arm (the `"` breaks
the anchor) *and* past the normalized keyword scan (the undecoded body hides
the verb). The fix splits the two requirements — the segment's
**command-position token alone** is run through the shared `anchor_unquote()`
to decide whether it is `git`, and the rest of the segment stays raw so the
`$'` marker survives to be seen. Segment splitting on `;&|()` reproduces what
the old regex's `(^|[;&|(])` start class and `[^;&|]*` body expressed.

#### Round 5 — the line-continuation axis (codex-1)

Reproduced on a native worker lane (`HIMMEL_WORKER=1`) against the round-4
hook. Another **live one-command bypass**, and the cheapest one yet: the
bypass and its denied twin are the same command, one keystroke apart.

| command | pre-fix | now |
|---|---|---|
| `git remote \`⏎`set-url origin /tmp/evil.git` | **rc=0** | rc=2 |
| `git remote \`␍⏎`set-url origin /tmp/evil.git` | **rc=0** | rc=2 |
| `git remote se\`⏎`t-url origin /tmp/evil.git` | **rc=0** | rc=2 |
| `git remote \`⏎`-v \`⏎`set-url origin /tmp/evil.git` | **rc=0** | rc=2 |
| `git remote set-url origin /tmp/evil.git` | rc=2 | rc=2 |

Bash **deletes** a backslash-newline pair before it parses anything, so every
row above executes the plain `git remote set-url …` the fence denies. The hook
flattened that newline to `;` *before* anything looked for a verb, so the
remote-verb tokeniser met a **segment break exactly where the verb belonged**,
found no verb after `remote`, and the write sailed through. The follow-up
`git fetch origin` is allowed by design, so one allowed command installs the
attacker's history as the trusted anchor.

**The fix is an ordering, and the ordering is the whole finding.** Line
continuations are normalised away in `join_line_continuations()` **upstream**
of the newline→`;` flattening that builds `anchor_cmd_lc` (and
therefore `anchor_cmd_norm`). Downstream is too late: by then `\`+newline has
already become `\;`, and nothing can distinguish it from a separator the
operator typed. It is an **axis removal** — reproducing bash's own deletion
rule closes every command this spelling can hide — not a seventh spelling
appended to a list. A single terminal `␍` is stripped from the line being
joined, because a CRLF checkout puts it *after* the backslash and the join
would otherwise never fire (the same byte that defeated the pin-dir scanner's
join in PR #2005).

**Parity, because the join can fail-open in the other direction.** `\\`+newline
is an escaped literal backslash: the line really ends and bash runs two
commands. Joining there would **erase a real boundary**, so
`echo x\\`⏎`git fetch . x:origin/main` would stop being a command-position
fetch and class (ii) would go blind. Only an **odd** trailing run continues —
the same rule the pin-dir scanner's `trailing_escapes()` applies. That
direction is a test row of its own, and it goes rc=0 (a manufactured
fail-open) when the odd/even test is replaced by "any trailing backslash".

A continuation is ordinary shell formatting, so what it reassembles into is
the only thing that may matter: a multi-line `git commit -m` and an `echo`
split across a continuation are re-measured at rc=0 and pinned as ALLOW
controls, so the six DENY rows cannot be satisfied by refusing every command
containing a backslash-newline.

**Both fences in the file are fixed, by ONE shared joiner.** The GLM-lane
shape matchers (`gp_shape` / `gu_shape` / `gh_shape` / `net_shape`) build
their own `cmd_lc` with the same `tr '\n\r' ';;'`, and had the identical gap —
`git \`⏎`push origin main` measured **rc=0** while the plain spelling measured
rc=2, because those patterns want `git` and the verb in one segment.
`join_line_continuations()` is defined once at top level and called upstream
of *both* flattenings; it is deliberately not copied, because a divergent
second copy is this ticket's recurring defect (round 3 normalised quoting for
class (ii) and left class (i)'s identical need unserved for a whole round). A
fix applied to one member of a twin pair is half a fix, and this section says
so elsewhere — shipping that sentence over a known live twin would refute it.
The GLM lane is currently **dropped**; that is recorded as a fact, and it is
explicitly *not* the reason the twin would have been safe to leave — a fence
left rotten while its lane is dormant is a fence that fails the day the lane
comes back.

On that lane the join also removes a **false DENY**: `gh \`⏎`issue list` used
to deny, because the split hid the `gh issue` carve-out from `gh_allow` while
`gh_shape` still counted the `gh`. Joined, it reads as the `gh issue list`
bash really runs and is allowed by design. Judging what the shell *assembles*
is the point of the join in both directions, and both directions are pinned.
The parity row is mirrored on this lane too, so the odd/even sub-decision is
test-held on both sides rather than only on the anchor class.

**Read this before adding a seventh enumerated case.** This fence's shape
matching has now been defeated on **six separate axes**, each found by the
round that followed the one which fixed its predecessor:

1. bare local-path remote spellings (`.`, `./…`, `../…`, `/…`);
2. quoted/escaped remotes (codex-6);
3. bare relative names (`..`) and then the *configured* remote itself, which
   ended the remote-argument enumeration entirely — class (ii) inverted to
   gate the refspec **destination**;
4. ordinary shell quoting across the whole class-(i) keyword set (round 3);
5. intervening options, and a quoted command token (round 4);
6. line continuations — `\`+newline and `\`␍`+newline — which broke the text
   into segments *before* the verb could be read, on **both** the anchor class
   and the GLM-lane shape matchers (round 5).

Every round that lost had fixed the previous axis by **adding members to a
list**. The two round-4 fixes deliberately do not: tokenising removes the
adjacency axis rather than naming the options that exploit it, and anchoring
on the stripped command token removes the quoting axis rather than naming the
quote forms. Round 5 does not either: joining continuations before
segmentation removes the axis rather than naming the two spellings that
exploit it. A future edit that answers a new bypass by appending one more
spelling to a pattern is repeating the move that has failed six times —
prefer removing the axis, and if that is genuinely impossible, say so here.
