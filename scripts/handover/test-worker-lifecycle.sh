...
trap 'rm -rf "$TMP"' EXIT
# arm-resume-macos-cron fix (round 2): every arm in this suite is real
# (no --dry-run anywhere) and takes the macOS/crontab backend on this Mac,
# writing generated runner files under
...
# fleet-size cap before every real (non-dry-run) arm. Without this, every
# case here that reaches that far would call the REAL fleet count against
# THIS machine's own ambient process table and could spuriously refuse
# (rc=22) on a host that genuinely has >= HIMMEL_FLEET_CAP `-n HIMMEL-*`
# sessions running - unrelated to what these cases actually test. Only the
# fleet count matters here (arm-resume.sh's gate ignores every other
# bank-preflight verdict), so no bank-cache stub is needed.
...
# HIMMEL-2642: this station has a real `at` binary, so schedule_arm's
# `linux)` case takes the `at`-heredoc path (arm-resume.sh ~5122), never
# _crontab_schedule -- there is no separate crontab-path gap to fix here.
# The same schtasks statelessness bug the /query stub above fixes existed
# on this path too: a stateless "discard the body, exit 0" `at` combined
# with an always-empty `atq` is inconsistent with the post-arm EXISTENCE
# verify, which re-queries via list_existing (atq + `at -c <id>`,
# arm-resume.sh ~2717-2771) right after the create and reads the empty
# answer as "the create registered nothing" -> rc=2 on every real
# (non-dry-run) `at` arm (T2/T6/T9 here all do real arms). Give `at`/`atq`/
# `atrm` real create+query+delete state, backed by a file per job under
# $TMP/at-jobs instead of the real atd queue.
AT_DB="$TMP/at-jobs"
mkdir -p "$AT_DB"
...
db="$AT_DB"
if [ "\${1:-}" = "-c" ]; then
    # at -c <job-id> -- print back the recorded job body. list_existing
    # greps this for the leading "# \$TASK_NAME" marker line.
    cat "\$db/\${2:-}" 2>/dev/null
    exit 0
fi
# Otherwise a submission (\`at -t <stamp>\`, job body on stdin). Record it
# under an incrementing id so atq/'at -c'/atrm below can round-trip it.
next=1
[ -f "\$db/.next-id" ] && next=\$(cat "\$db/.next-id")
cat > "\$db/\$next"
echo \$((next + 1)) > "\$db/.next-id"
exit 0
EOF
...
for f in "\$db"/[0-9]*; do
    [ -f "\$f" ] || continue
    printf '%s\tThu Jan  1 00:00:00 2026 a testuser\n' "\$(basename "\$f")"
done
exit 0
EOF
# atrm: delete_existing's linux/at branch (arm-resume.sh ~2883) calls this on
# a --force replace's post-commit reap (T6 exercises it). Without a stub this
# fell through to PATH's real system atrm -- a hermetic violation, since it
# would query/touch the REAL atd queue for a job id that only happens to
# collide with one of ours. Remove the recorded file instead; exit 1 (like
# real atrm on an unknown job) when there is nothing to remove.
cat > "$SCHED_STUB/atrm" <<EOF
#!/usr/bin/env bash
f="$AT_DB/\${1:-}"
[ -f "\$f" ] || exit 1
rm -f "\$f"
...
# schtasks AND at stubs' state; without --force here, this arm would
# otherwise (and correctly) hit the real dedup-block path (rc=3) instead of
# the scenario this case means to exercise. Reset both so this arm starts
