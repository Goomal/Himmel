...
check_c15() {
    ...
    # HIMMEL-2733: onDemandPlugins is the ON-DEMAND tier (installed, shipped
    # disabled) — a key from it that the operator has intentionally enabled
    # must not be counted as drift below. Same non-object shape-check as
    # enabledPlugins above: a malformed onDemandPlugins skips the whole check
    # rather than risking has($k) erroring into a false OK.
    case "$(jq -r '.onDemandPlugins | type' "$tmpl" 2>/dev/null)" in
        object|null) ;;
        *) emit INFO C15-plugins "lean template has a non-object onDemandPlugins — drift not checked"; return ;;
    esac
    tmpl_ep="$(jq -c '.enabledPlugins // {}' "$tmpl")"
    live_ep="$(jq -c '.enabledPlugins // {}' "$SETTINGS")"
    local tmpl_od; tmpl_od="$(jq -c '.onDemandPlugins // {}' "$tmpl")"
    ...
    # settings.local.json AND not an onDemandPlugins spec. ANY local entry
    # (true OR false) is an explicit operator override, so it is never drift
    # — a local `false` means the operator already disabled it on purpose.
    # An onDemandPlugins spec the operator enabled is likewise never drift —
    # it's the ON-DEMAND tier working as designed — but it IS reported below
    # via on_demand_enabled, informationally.
    drift="$(jq -rn --argjson t "$tmpl_ep" --argjson l "$live_ep" --argjson lo "$local_ep" --argjson od "$tmpl_od" '
        [ $l | to_entries[] | select(.value != false)
          | .key as $k | select( (($t[$k]) != true) and (($lo | has($k)) | not) and (($od | has($k)) | not) ) | $k ] | .[]' 2>/dev/null || true)"
    count="$(printf '%s\n' "$drift" | grep -c . || true)"
    # Current user-scope runtime state comes from settings.json. The sibling
    # settings.local.json above is himmel reconciliation input, not a Claude
    # Code runtime layer; reconcile-enabled-plugins copies its values into the
    # target settings.json before they become live.
    local on_demand_enabled on_demand_count
    on_demand_enabled="$(jq -rn --argjson l "$live_ep" --argjson od "$tmpl_od" '
        [ $l | to_entries[] | select(.value != false) | .key as $k | select($od | has($k)) | $k ] | .[]' 2>/dev/null || true)"
    on_demand_count="$(printf '%s\n' "$on_demand_enabled" | grep -c . || true)"
    if [ "${on_demand_count:-0}" -gt 0 ]; then
        local od_list; od_list="$(printf '%s' "$on_demand_enabled" | paste -sd, - 2>/dev/null | sed 's/,/, /g')"
        [ -n "$od_list" ] || od_list="$(printf '%s' "$on_demand_enabled" | tr '\n' ' ')"
        emit INFO C15-plugins-ondemand "$on_demand_count on-demand plugin(s) intentionally enabled (informational, not drift): $od_list"
    fi
    if [ "${count:-0}" -eq 0 ]; then
